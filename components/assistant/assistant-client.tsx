...
export function AssistantClient() {
  ...
  return (
    <section className="flex h-full min-h-0 flex-col overflow-hidden bg-background">
      <div className="hidden items-center gap-3 border-b bg-primary/[0.04] px-4 py-3 lg:flex lg:px-6">
        ...
        <p className="text-sm font-semibold">Cost Track Assistant</p>
      </div>
      <div className="flex-1 space-y-5 overflow-y-auto p-4 sm:p-6" aria-live="polite">
        {messages.length === 0 ? <div className="mx-auto flex max-w-xl flex-col items-center py-8 text-center sm:py-16"><span className="grid size-16 place-items-center rounded-2xl bg-primary/10 text-primary ring-8 ring-primary/[0.03]"><Sparkles className="size-7" /></span><h2 className="mt-6 font-heading text-2xl font-semibold">Tanyakan keuanganmu</h2><div className="mt-7 grid w-full gap-2 sm:grid-cols-2">{initialSuggestions.map((suggestion) => <Button key={suggestion} className="justify-start rounded-xl bg-background" onClick={() => void ask(suggestion)} type="button" variant="outline"><Sparkles className="text-primary" />{suggestion}</Button>)}</div></div> : messages.map((message, index) => message.role === "user" ? <div key={index} className="ml-auto flex max-w-[88%] flex-row-reverse items-start gap-2"><span className="grid size-8 shrink-0 place-items-center rounded-lg bg-muted"><UserRound className="size-4" /></span><p className="rounded-2xl rounded-tr-sm bg-primary px-4 py-3 text-sm leading-6 text-primary-foreground shadow-sm">{message.text}</p></div> : <div key={index} className="flex max-w-[94%] items-start gap-2"><span className="grid size-8 shrink-0 place-items-center rounded-lg bg-primary/10 text-primary"><Bot className="size-4" /></span><div className="rounded-2xl rounded-tl-sm border bg-card p-4 shadow-xs"><p className="text-sm leading-6">{message.data.answer}</p>{message.data.metric ? <p className="mt-2 font-heading text-2xl font-semibold tracking-tight">{message.data.metric}</p> : null}{message.data.details?.length ? <ul className="mt-3 space-y-1.5 text-sm text-muted-foreground">{message.data.details.map((detail) => <li key={detail} className="flex gap-2 before:mt-2 before:size-1 before:shrink-0 before:rounded-full before:bg-primary">{detail}</li>)}</ul> : null}<div className="mt-4 flex flex-wrap gap-2">{message.data.suggestions.slice(0, 3).map((suggestion) => <Button key={suggestion} className="rounded-full" onClick={() => void ask(suggestion)} size="sm" type="button" variant="outline">{suggestion}</Button>)}</div></div></div>)}
